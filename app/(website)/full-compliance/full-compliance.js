...
export default function FullCompliance() {
  ...
  const [showPayments, setShowPayments] = useState(false); // Nuevo estado para Payments
  ...
  const handleAnswer = () => {
    ...
    if (filteredQuestions[currentQuestion].type === "number") {
      ...
      // Solo un bloque de validación para asegurar que el mensaje de error no se duplique
      if (
        !/^\d+$/.test(value) ||
  ...
  const calculatePlanillaCost = () => {
    if (answers.manejoPlanilla === "Si") {
      if (answers.colaboradores > 5) {
        return 10000 * (answers.colaboradores - 5) * 1.13; // Costo por cada colaborador adicional
      } else {
        return 0; // No hay costo adicional si hay 5 o menos colaboradores
      }
    } else {
      return 0; // Si no maneja planilla, el costo es 0
    }
  };
      ...
      <motion.div
        ...
        {currentQuestion < filteredQuestions.length ? (
          ...
          <CustomerInfoForm
            ...
            selectedPlan={"Full Compliance Plan"}
            calculateBaseCost={calculateBaseCost}
            calculatePlanillaCost={calculatePlanillaCost}
            calculateFacturasCost={calculateFacturasCost}
            calculateTotalCost={calculateTotalCost}
